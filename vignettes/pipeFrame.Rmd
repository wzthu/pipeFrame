...
Bioc 3.9 built on R 3.5. To install the latest version of pipeFrame, 
...
For package developers, as the function `.onload` is called 
automatically when a new package is loading, the pipeFrame 
initialization is set when the function `.onLoad` is invoked as follows.
...
including genome, job name, reference directory, temporary directory, 
...
testInputBedFilePath <- system.file(package = "pipeFrame", "extdata",
                                    "testRegion.bed")
...
result2 <- runOverlappedRandomRegion(inputBed = testInputBedFilePath, 
                                     randomBed = testInputBedFilePath)

```

The classes `RandomRegionOnGenome` and `OverlappedRandomRegion` 
will be implemented and introduced in next section.
...
`.Object <- setInput(.Object,'itemname', value)`, 
`.Object <- setOutput(.Object,'itemname', value)` and
`.Object <- setParam(.Object,'itemname', value)` rather than
...
```{r}
examplePipe(sampleNumb = 1000,inputBed = system.file(package = "pipeFrame", "extdata","testRegion.bed"),genome = "hg19",
